...
| Total CI-wait duration                       | 433 seconds               |
| Total Vercel-wait duration                   | 303 seconds               |
...
- Observed/configuration corrected: 2026-08-17T19:02:44Z / 2026-08-17T19:05:20Z. Phase/origin: P6; self-introduced configuration omission after the visibility transition.
- Native dependency review reported that it was unsupported, while the independent locked production audit remained green.
- Correction: enabled the exact repository dependency graph together with no-cost secret scanning and push protection; automatic dependency-update writes remain disabled.
- Invalidated proof: `f1a208c` dependency review and merge. Closing native CI rerun remains required.

### ISS-019 — SBOM generator used partial package-name encoding

- Observed/local correction: 2026-08-17T19:03:42Z / 2026-08-17T19:07:03Z. Phase/origin: P6; self-introduced.
...
- Closing verification: full local release gate passed; exact-head persisted Code Scanning remains required. Status: Resolved locally, pending CI.
...
- Closing verification: the three snapshot alerts are dismissed while the real generator alert remains visible until the correction commit is scanned. Status: Resolved.
...
- Corresponding issue: ISS-019. Responsible commit: `5d0cba19f21a539e133a8ef079b0c2ade7eac93b`; correction commit pending.
- Detected/local correction: 2026-08-17T19:03:42Z / 2026-08-17T19:07:03Z.
...
- Proof invalidated: `f1a208c` Code Scanning, SBOM, and merge. Closing verification: full component encoding and regenerated SBOM pass locally; public scan pending.
...
Running. Repository foundation, six-app implementation, local and clean-clone release checks, dual protocol matrices, public readiness, and exact-head DEP-003 Preview proof have passed. The repository is public, dependency graph plus secret protections are enabled, and Vercel is linked with Production Branch `forward`. The first public native dependency review and Code Scanning policy correctly blocked `f1a208c`; dependency graph configuration, complete PURL encoding, frozen-source alert dispositions, and an exact-head post-release approval adapter are locally complete and awaiting the next public CI/native Preview run. No shared organization rule was weakened. Earlier failed/incomplete deployments remain truthfully recorded. The 24-hour observation remains Pending Operational Observation outside the fixed 20-minute gate.
